...
/*   Updated: 2018/10/05 13:44:43 by aanzieu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
FragTrap::FragTrap(void)
{
    ...
    srand(time(0));
	ClapTrap::set_Stats("* FR4G-TPAP *", 1, 100, 100, 100, 100, 30, 20, 5);
    std::cout << "* FR4G-TPAP * Default Constructor Called! <" << this->_Name << ".Fragtap> : Hey everybody! Check out my package!" << std::endl;
    return;
}
...
{
    srand(time(0));
	ClapTrap::set_Stats(name, 1, 100, 100, 100, 100, 30, 20, 5);
    std::cout << "* FR4G-TPAP * Parameter Constructor Called! <" << this->_Name << ".Fragtap> : Let's get this party started!" << std::endl;
    return;
}
...
FragTrap::~FragTrap(void)
{
    std::cout << "* FR4G-TPAP * Destructor Called: <" << this->_Name << ".Fragtap> I'M DEAD I'M DEAD OHMYGOD I'M DEAD!" << std::endl;
    return;
}
...
FragTrap & FragTrap::operator=( FragTrap const & rhs )
{
    ...
    std::cout << "* FR4G-TPAP * Assigning Operator = Called : <" << this->_Name << ".Fragtap> Thanks for giving me a second chance, God. I really appreciate it." << std::endl
              << std::endl;
    return (*this);
}
...
void	FragTrap::vaulthunter_dot_exe( std::string const &target )
{
	...
	if (ClapTrap::get_Energy_points() < 25)
	{
		std::cout << "* FR4G-TPAP * <" << this->_Name << "> does not have enough energy points! Current energy " <<
			ClapTrap::get_Energy_points() << " / " << ClapTrap::get_Max_energy_points() << std::endl << std::endl;
		return;
	}
	else
		ClapTrap::set_Energy_points(ClapTrap::get_Energy_points() - 25);
	std::cout << "* FR4G-TPAP * <" << this->_Name << "> uses VaultHunter.Exe! Let's see it's real powers!" << std::endl;
	i = (i % 5);
	while (x < 5)
	{
		if (x == i)
...
{
	std::cout << "* FR4G-TPAP * <" << this->_Name << "> uses One Shot Wonder! The effect extends to Claptrap's allies!" << std::endl
              ...
              << target << " takes " << ClapTrap::get_Hit_points() * 5.0f << " damage !" << std::endl
...
{
	(void)target;
	ClapTrap::set_Max_hit_points(ClapTrap::get_Max_hit_points() * 1.5);
	std::cout << "* FR4G-TPAP * <" << this->_Name << "> uses Rubber Ducky!." << std::endl
              << "With his rubber ducky his max health has increased by 1.5x!" << std::endl 
              <<" New Max Health " << ClapTrap::get_Max_hit_points() << std::endl
...
{
	(void)target;
	ClapTrap::set_Armor_damage_reduction(ClapTrap::get_Armor_damage_reduction() * 5);
	std::cout << "* FR4G-TPAP * <" << this->_Name << "> deploys a Holotrap, add Armor reduction damage and becomes invisible" << std::endl
...
{
	std::cout << "* FR4G-TPAP * <" << this->_Name << "> uses Funzerker!" << std::endl
              << "Get copy of his currently equipped weapon in his other clamp" << std::endl
              << "+50%% fire rate and +100%% reload speed."
		      << target << " takes " << ClapTrap::get_Ranged_attack_damage() * 0.5f << " damage !" << std::endl
...
{
	std::cout << "* FR4G-TPAP * <" << this->_Name << "> uses Mechromagician !" << std::endl 
              << "Gun damage is increased by approximately 263%%" << std::endl
              << target << " takes " << ClapTrap::get_Ranged_attack_damage() * 2.63f << " damage !" << std::endl
