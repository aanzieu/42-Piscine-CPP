...
/*   Updated: 2018/10/09 17:38:41 by aanzieu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Bureaucrat OfficeBlock::attribSigner(void)
{
    return (*_signer);
}

Bureaucrat OfficeBlock::attribExecuter(void)
{
    return (*_executer);
}
int OfficeBlock::getAll(void)
{
   return (!this->_intern || !this->_signer || !this->_executer ? 0 : 1);
}
int OfficeBlock::getIntern(void)
{
    return (!this->_intern ? 0 : 1);
}
int OfficeBlock::getSigner(void)
{
    return (!this->_signer ? 0 : 1);
}
int OfficeBlock::getExecuter(void)
{
    return (!this->_executer ? 0 : 1);
}
